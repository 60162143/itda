...
public class MyPageBookmarkStoreRvAdapter extends RecyclerView.Adapter<MyPageBookmarkStoreRvAdapter.CustomStoreViewHolder> {
    ...
    private ArrayList<MyPageBookmarkStoreData> Stores; // 가게 데이터
    ...
    public void onBindViewHolder(@NonNull CustomStoreViewHolder holder, int position) {
        ...
        holder.bookmarkStoreName.setText(store.getStoreName());   // 가게 이름 SET

        // 거리가 10m 이상인 경우만 거리 표시
        if(store.getStoreDistance() <= 0.01){
            holder.bookmarkStoreDistance.setText("10m 이내");
        }else{
            holder.bookmarkStoreDistance.setText(String.format("%.2f", store.getStoreDistance()) + "km");
        }

        holder.bookmarkStoreStarScore.setText(String.format("%.1f", store.getStoreScore()));  // 가게 별점 SET
        holder.bookmarkStoreInfo.setText(store.getStoreInfo());   // 가게 간단 정보 SET

        holder.bookmarkStoreHashTag.setText(store.getStoreHashTag());   // 추후 추가
    }
