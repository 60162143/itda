...
import com.example.itda.ui.info.onInfoPhotoTotalRvClickListener;
...
public class MyPagePhotoRvAdapter extends RecyclerView.Adapter<MyPagePhotoRvAdapter.CustomMyPagePhotoViewHolder>{

    private final ArrayList<infoPhotoData> Photos;    // 사진 데이터

    // 리사이클러뷰 클릭 리스너 인터페이스
    private static onMyPagePhotoRvClickListener rvClickListener = null;
    ...
    // Constructor
    public MyPagePhotoRvAdapter(Context context, onMyPagePhotoRvClickListener clickListener, ArrayList<infoPhotoData> photos){
    ...
    @NonNull
    @Override
    public CustomMyPagePhotoViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        // layoutInflater로 xml객체화. viewHolder 생성
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.rv_info_photo_total, parent, false);

        return new MyPagePhotoRvAdapter.CustomMyPagePhotoViewHolder(view);
    }
    ...
    @Override
    public void onBindViewHolder(@NonNull CustomMyPagePhotoViewHolder holder, int position) {
    ...
    public static class CustomMyPagePhotoViewHolder extends RecyclerView.ViewHolder {
        ImageView photoImage;   // 사진 이미지
        ImageView photoDelBtn;   // 사진 삭제 버튼

        public CustomMyPagePhotoViewHolder(@NonNull View itemView) {
            super(itemView);

            photoImage = itemView.findViewById(R.id.info_photo_total_image);
            photoDelBtn = itemView.findViewById(R.id.info_photo_total_del_btn);
            ...
            photoImage.setOnClickListener(view -> {
                ...
                if(pos != RecyclerView.NO_POSITION){
                    rvClickListener.onMyPagePhotoRvClick(view, getAbsoluteAdapterPosition(), "image");
                }
            });

            // 삭제 버튼 클릭 이벤트 인터페이스 구현
            photoDelBtn.setOnClickListener(view -> {
                int pos = getAbsoluteAdapterPosition(); // 현재 position

                // 리스너 객체를 가진 Activity에 오버라이딩 된 클릭 함수 호출
                if(pos != RecyclerView.NO_POSITION){
                    rvClickListener.onMyPagePhotoRvClick(view, getAbsoluteAdapterPosition(), "delete");
                }
            });
        }
    }
