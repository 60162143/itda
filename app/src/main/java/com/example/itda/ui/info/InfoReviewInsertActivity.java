...
public class InfoReviewInsertActivity extends AppCompatActivity implements onInfoReviewInsertPhotoRvClickListener{
    ...
    // Data
    private final ArrayList<infoReviewPhotoFileData> PhotosFiles = new ArrayList<>();   // 리뷰 사진 파일 데이터 ( FTP에 업로드 하기 위해 )
    private final ArrayList<Uri> uriList = new ArrayList<>();   // 이미지의 uri를 담을 ArrayList 객체
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        String IMAGE_PLUS = ((globalMethod) getApplication()).getImagePlusPath();   // 추가할 이미지 기본 화면 정보 ( 회색 화면에 가운데 플러스 아이콘 있는 화면 )
            ...
            if(result.getResultCode() == RESULT_OK){ // 사진 선택 완료
                ...
                if(data == null){   // 어떤 이미지도 선택하지 않은 경우
                    ...
                    if(data.getClipData() == null){ // 이미지를 하나만 선택한 경우
                        ...
                        // 사진 개수 SET
                        String infoReviewInsertPhotoCntTxt = "( " + (uriList.size() - 1) + " / 10 )";
                        infoReviewInsertPhotoCnt.setText(infoReviewInsertPhotoCntTxt);
        ...
        // 첫번째 별점 클릭 리스너
        infoReviewInsertStar01.setOnClickListener(view -> {
            starScore = 1;  // 현재 별점
        ...
        // 두번째 별점 클릭 리스너
        infoReviewInsertStar02.setOnClickListener(view -> {
            starScore = 2;  // 현재 별점
        ...
        // 세번째 별점 클릭 리스너
        infoReviewInsertStar03.setOnClickListener(view -> {
            starScore = 3;  // 현재 별점
        ...
        // 네번째 별점 클릭 리스너
        infoReviewInsertStar04.setOnClickListener(view -> {
            starScore = 4;  // 현재 별점
        ...
        // 다섯번째 별점 클릭 리스너
        infoReviewInsertStar05.setOnClickListener(view -> {
            starScore = 5;  // 현재 별점
        ...
        // 작성 완료 버튼 클릭 리스너
        infoReviewInsertBtn.setOnClickListener(view -> {
            if(!TextUtils.isEmpty(infoReviewContent.getText().toString())){
                insertInfoReview(StoreId, UserId);  // 리뷰 데이터 Insert
    ...
    public void onInfoReviewInsertPhotoRvClick(View v, int position, String flag) {
        if(position == uriList.size() - 1){ // 사진 추가 이벤트
            ...
            if(flag.equals("delete")){    // 사진 삭제 버튼 클릭
                uriList.remove(position);   // 사진 URI 데이터 제거
        ...
        StringRequest insertReviewRequest = new StringRequest(Request.Method.POST, HOST + INSERT_REVIEW_PATH, response -> {
            try {
                ...
                if(!TextUtils.isEmpty(success) && success.equals("1")) {
                    ReviewId = jsonObject.getInt("reviewId");   // 추가한 리뷰 고유 아이디
                    ...
                    if(uriList.size() > 1) {
                        for(int i = 0; i < uriList.size() - 1; i++){
                            ...
                            if (uriList.get(i).getScheme().equals("content")) {
                                ...
                                try (Cursor cursor = getContentResolver().query(uriList.get(i)   // Uri : 찾고자하는 데이터의 Uri, 접근할 앱에서 정의, 내 앱에서 만들고 싶다면 manifest에서 만들 수 있음
                        ...
                        nThread.join(); // Thread가 종료될 때까지 대기
                    }
    ...
    class NThread extends Thread{
        ArrayList<infoReviewPhotoFileData> photosFiles; // 리뷰 사진 파일 데이터
    ...
    public void uploadFile(File file, int reviewId, String name, String ets, double size){
        ...
        try {
            ...
            handler.post(() -> insertInfoReviewPhoto(reviewId, name, ets, size));   // 리뷰 사진 데이터 Insert

        } catch (Exception e) {
            handler.post(() -> StyleableToast.makeText(getApplicationContext(), "사진 저장 실패", R.style.blueToast).show());
