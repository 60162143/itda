...
public class MyPageBookmarkActivity extends AppCompatActivity implements onMyPageBookmarkStoreRvClickListener, onMyPageBookmarkCollaboRvClickListener {
    ...
    // Data
    private final ArrayList<myPageBookmarkStoreData> bookmarkStoreData = new ArrayList<>();   // 유저 찜한 가게 데이터
    private ArrayList<mainBookmarkStoreData> infoBookmarkStoreData = new ArrayList<>(); // 유저 찜한 가게 간단 데이터 ( intent용 )
    private final ArrayList<myPageBookmarkCollaboData> bookmarkCollaboData = new ArrayList<>();   // 유저 찜한 협업 목록 데이터

    // Login Data
    private SharedPreferences User; // 로그인 데이터 ( 전역 변수 )
    private boolean isGpsPossible = false;  // 위치 정보 동의 여부
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        bookmarkStoreBtn.setOnClickListener(view -> {
            ...
            bookmarkStoreBtn.setBackgroundResource(R.drawable.round_select_green_10dp);
            bookmarkCollaboBtn.setEnabled(true);
            bookmarkCollaboBtn.setBackgroundResource(R.drawable.round_unselect_green_10dp);
        ...
        bookmarkCollaboBtn.setOnClickListener(view -> {
            ...
            bookmarkStoreBtn.setBackgroundResource(R.drawable.round_unselect_green_10dp);
            bookmarkCollaboBtn.setEnabled(false);
            bookmarkCollaboBtn.setBackgroundResource(R.drawable.round_select_green_10dp);
            ...
            try {
                ...
                if(bookmarkStoreArr.length() > 0){
                    for(int i = 0; i < bookmarkStoreArr.length(); i++){
                        ...
                        myPageBookmarkStoreData bookmarkStore = new myPageBookmarkStoreData(
    ...
    private void getBookmarkCollaboData(){
        ...
        StringRequest bookmarkCollaboRequest = new StringRequest(Request.Method.GET, HOST + bookmarkCollaboPath, response -> {
            try {
                JSONObject jsonObject = new JSONObject(response);                 // Response를 JsonObject 객체로 생성
                JSONArray bookmarkCollaboArr = jsonObject.getJSONArray("bookmarkCollabo");   // 객체에 bookmakrCollabo라는 Key를 가진 JSONArray 생성
                ...
                if(bookmarkCollaboArr.length() > 0){
                    for(int i = 0; i < bookmarkCollaboArr.length(); i++){
                        ...
                        myPageBookmarkCollaboData bookmarkCollabo = new myPageBookmarkCollaboData(
                                object.getInt("bookmarkCollaboId")  // 찜한 협업 리스트 고유 아이디
    ...
    public void onMyPageBookmarkStoreRvClick(View v, int position, String flag) {
        if(flag.equals("delete")){  // 찜 목록 삭제 버튼 클릭
            ...
            dlConfirmBtn.setOnClickListener(view -> {
                // POST 방식 파라미터 설정
                // Param => bmkStId : 변경할 찜한 가게 목록 테이블 고유 아이디
                Map<String, String> param = new HashMap<>();
                param.put("bmkStId", String.valueOf(bookmarkStoreData.get(position).getBookmarkStoreId()));   // 변경할 찜한 가게 목록 테이블 고유 아이디
    ...
    public void onMyPageBookmarkCollaboRvClick(View v, int position, String flag) {
        switch (flag) {
            case "delete":   // 찜 목록 삭제 버튼 클릭
                ...
                dlConfirmBtn.setOnClickListener(view -> {
                    // POST 방식 파라미터 설정
                    // Param => bmkCobId : 변경할 찜한 협업 목록 테이블 고유 아이디
                    Map<String, String> param = new HashMap<>();
                    param.put("bmkCobId", String.valueOf(bookmarkCollaboData.get(position).getBookmarkCollaboId()));    // 변경할 찜한 협업 목록 테이블 고유 아이디
    ...
    public void getStoreData(int storeId){
        ...
        storePath += String.format("?storeId=%s", storeId); // 가게 고유 아이디
        ...
        StringRequest mainStoreRequest = new StringRequest(Request.Method.GET, HOST + storePath, response -> {
            try {
                ...
                mainStoreData mainStore = new mainStoreData(
                        ...
                        , isGpsPossible ? distance / 1000 : 0);   // 현위치에서 가게까지의 거리
                ...
                intent.putParcelableArrayListExtra("bookmarkStore", infoBookmarkStoreData); // 유저 찜한 가게 목록
                intent.putExtra("pageName", "MyPageBookmarkActivity");  // 화면전환 페이지 명
