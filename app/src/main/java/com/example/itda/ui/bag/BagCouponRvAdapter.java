...
public class BagCouponRvAdapter extends RecyclerView.Adapter<BagCouponRvAdapter.CustomMainCategoryViewHolder>{
    private final ArrayList<bagCouponData> Coupons;  // 사용가능 쿠폰 데이터
    ...
    public void onBindViewHolder(@NonNull BagCouponRvAdapter.CustomMainCategoryViewHolder holder, int position) {
        bagCouponData coupon = Coupons.get(position);     // 현재 position의 쿠폰 정보
        ...
        holder.storeName.setText(coupon.getStoreName()); // 가게 이름 SET
        ...
        if(coupon.getDiscountRate() < 10){
            spannableCouponDiscountRateTxt.setSpan(new ForegroundColorSpan(Color.parseColor("#5492D3")), 0, 2, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            spannableCouponDiscountRateTxt.setSpan(new RelativeSizeSpan(1.5f), 0, 1, SpannableString.SPAN_EXCLUSIVE_EXCLUSIVE);
        }else{
            spannableCouponDiscountRateTxt.setSpan(new ForegroundColorSpan(Color.parseColor("#4AD5A7")), 0, 3, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            spannableCouponDiscountRateTxt.setSpan(new RelativeSizeSpan(1.5f), 0, 2, SpannableString.SPAN_EXCLUSIVE_EXCLUSIVE);
        }

        // 쿠폰 할인 율 SET
        holder.couponDiscountRate.setText(spannableCouponDiscountRateTxt);

        // 쿠폰 만료일 SET
        String couponExpireDateTxt = "사용 가능 기한 : " + coupon.getExpDate() + " 까지";
        holder.couponExpireDate.setText(couponExpireDateTxt);
    }
